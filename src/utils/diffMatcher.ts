...
export function findBestMatch(haystack: string, needle: string, threshold: number = 0.8): MatchResult | null {
  // まず完全一致を試す
 ...
 */
function findOriginalPosition(original: string, _normalized: string, normalizedPos: number): number {
  let originalPos = 0;
 ...
 * 元のテキストで必要な長さを計算
 */
function findOriginalLength(original: string, startPos: number, targetLength: number): number {
  let length = 0;
  let count = 0;
